  ...
  def schedule_actions
    ...
    if can? :update, AppSettings
      if AppSettings.auto_mail == 'on'
        actions << ["AutoMail ON", setting_path('off'), {:method => :put, :title => "click to disable it", :class => "btn btn-success"}]
        actions << ["Config/Check Mail Content", settings_path]
      else
        actions << ["AutoMail OFF", setting_path('on'), {:method => :put, :title => "click to enable it", :class => "btn btn-danger"}]
      end
    end
